<script lang="ts" module>
    ...
    /**
 * 创建一个防抖函数，在连续调用时延迟执行，并可选择首次立即执行。
 * 
 * @param func - 要防抖的原始函数
 * @param threshold - 延迟毫秒数，默认 100
 * @param execAsap - 是否在第一次调用时立即执行（后续调用在延迟内不会执行），默认 false
 * @returns 防抖后的函数
 */
export function debounce<F extends (...args: any[]) => any>(
  ...
  // 定时器句柄（浏览器中为 number，Node.js 中为 Timeout 对象）
...
<script lang="ts">
    ...
    let on_change = $derived(bounce ?  debounce(handle_change, bounce):handle_change );
